...
class MyPrompt(Cmd):
    ...
    def cmd_ui(self):
        ...
        print("*" + (' ' * 5) + 'Firmware Version: %s' % r)
        print("*" + (' ' * 5) + 'Return Delay Time: %s' % r)
        print("*" + (' ' * 5) + 'Status Return Level: %s' % r)
        print("*" + (' ' * 5) + 'Goal Position (radian): %s' % self.servo.get_angle())
        print("*" + (' ' * 5) + 'Moving Speed (rpm): %s' % self.servo.get_speed())
        print("*" + (' ' * 5) + 'Temperature: %s°C' % r)
...
id angle speed
    ...
    def do_action(self, args):
        ...
        for i in range(0, 6):
            ...
            print("*" + (' ' * 5) + 'Current Angle: ' + (color.LIGHTMAGENTA_EX + '%s' + color.RESET + '°C') % r)
            print("*" + (' ' * 5) + 'Current Speed: ' + (color.LIGHTMAGENTA_EX + '%s' + color.RESET + '°C') % r)
